...
- **文字面试**：选择项目深挖、后端技术或协作表达，免费模式练习 3、5 或 8 题，付费模式每场最多 5 题。根据实际回答继续追问，保存逐题反馈与总结，也可提前结束并复盘已答部分。支持复盘下载和针对薄弱点再练；最多保存 6 场练习。

新增资料继续使用原报告的访问权限、恢复码和到期时间。面试保存开始时的简历快照；后续编辑版本不会改变历史面试。另支持账号、个人收款码、人工核款发放次数和退款登记；购买默认关闭。当前没有图片 OCR、公开分享、自动支付回调或订阅会员。

项目文档：[计划书](docs/plan.md) · [求职准备设计](docs/preparation.md) · [人工收款与次数](docs/billing.md) · [部署指南](DEPLOY.md) · [验收记录](docs/verification.md)。
...
管理后台入口为 `/#admin`，用户管理为 `/#admin/users`，使用服务器配置的独立 `BILLING_ADMIN_KEY` 登录。可搜索和筛选用户，查看免费体验、已购次数、订单及登录情况，保存管理备注、补发未领取过的四项体验、限制或恢复 AI 生成、退出全部设备；操作保留记录。真实收款码、套餐价格、核款说明与联系方式都在管理页设置；配置完整后再开启购买，详见 [人工收款指南](docs/billing.md)。

新注册账号自动获得诊断、精修、岗位适配和文字面试各一次免费体验，优先于已购次数使用。精修含追问和一次改写，面试含最多 5 题及复盘；每个账号仅赠送一次，最终生成失败退回次数。账号页可查看体验余额及来源记录。

## 数据处理

- 匿名和账号 Cookie 用于会话识别，数据库存储令牌摘要；真实接口有会话授权和 CSRF 校验。账号密码加盐哈希保存，账号恢复码与报告恢复码分开。
- 账号、订单、退款、次数和管理操作记录独立保留，不随报告到期删除。交易明细、收款码、订单快照、管理备注及操作原因加密保存；次数目前不设到期日。
...
37 组测试覆盖私有访问、恢复及撤销、引用来源、输入边界、同意与 CSRF、加密与到期、任务重试 / 租约恢复、并发重复提交、运行中删除、上游协议和错误处理，以及简历逐条采纳、岗位版本隔离、事实纠正、面试快照、连续回答、提前复盘、薄弱点重练和旧库升级。账务测试覆盖账号恢复与会话轮换、人工核款、交易单号去重、并发预占、失败退回、精修 / 面试整轮计次、退款暂停和部分退款、收款码校验，以及新用户只赠送一次、赠送优先和退款隔离。用户管理测试覆盖权限与 CSRF、搜索分页、订单归属、补发并发与回滚、生成限制、加密备注、操作冲突、指定账号会话撤销，以及旧库到 v5 的迁移。上游协议测试使用本地替身，不发出真实 API 请求。
