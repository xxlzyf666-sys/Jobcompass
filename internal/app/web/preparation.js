...
export class PreparationUI {
  ...
  refinement(v) {
    ...
    const questions = v.questions || [], edits = v.edits || [];
    return `<div class="prep-grid">${this.source(v)}<div class="prep-main">
      ...
      <p class="prep-description">针对当前简历和岗位，追问职责、技术取舍和可验证结果。没有的经历和数据，请直接说明。</p>
      ${questions.length ? `<form data-form="facts" data-draft="facts"><fieldset ${this.blocked ? 'disabled' : ''}>${questions.map((q,index) => `<article class="prep-question"><div class="prep-question-label"><span>追问 ${index+1}</span><span>${h(q.why)}</span></div><blockquote>${h(q.quote)}</blockquote><label for="fact-${h(q.id)}">${h(q.question)}</label><textarea id="fact-${h(q.id)}" name="${h(q.id)}" rows="3" minlength="2" maxlength="1600" required placeholder="补充你实际负责的工作；不确定的信息请如实说明。">${h(answers[q.id] ?? q.answer)}</textarea></article>`).join('')}<label class="prep-check"><input type="checkbox" name="confirmed" required ${answers.confirmed ? 'checked' : ''}><span>我确认补充内容真实，同意用于本次简历精修。</span></label><p class="prep-caption" data-unsaved ${this.drafts.has(this.draftKey('facts')) ? '' : 'hidden'}>回答尚未提交，生成时会一起保存。</p><button type="submit" class="button primary" ${this.aiBlocked ? 'disabled' : ''}>确认补充，生成修改建议 ↗</button></fieldset></form>` : `<div class="prep-invitation"><p>让问题具体到你的经历。回答后，再一起整理表达。</p><button class="button primary" data-prep="questions" ${this.aiBlocked ? 'disabled' : ''}>生成针对我的追问 ↗</button></div>`}</section>
  ...
  interviews(v) {
    ...
    const newForm = `<section class="prep-panel"><h2>开始一场针对你的练习</h2><p class="prep-description">使用“${h(v.name)}”的当前成稿和目标岗位。面试开始后，题目与复盘始终基于这份快照。</p><form data-form="interview"><fieldset ${this.aiBlocked ? 'disabled' : ''}><label class="prep-field" for="interview-focus">练习方向<select id="interview-focus" name="focus"><option value="project">项目深挖</option><option value="backend">后端技术</option><option value="behavioral">协作表达</option></select></label><label class="prep-field" for="interview-rounds">本轮题数<select id="interview-rounds" name="rounds"><option value="3">3 题 · 快速练习</option><option value="5" selected>5 题 · 完整一轮</option><option value="8">8 题 · 深入练习</option></select></label><button class="button primary full-width" type="submit">开始文字模拟面试 ↗</button></fieldset></form></section>`;
  ...
  async click(event) {
    ...
    try {
      ...
      if (action === 'questions' && this.version.questions?.length && !window.confirm('重新生成会替换当前追问和修改建议，已采纳的正文及已确认事实会保留。继续吗？')) return;
      ...
      if (action === 'practice_again') { await this.change('start_interview', { interview_id: this.interview.id, rounds: this.interview.rounds, focus: this.interview.focus }); return; }
