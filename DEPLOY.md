...
先停止服务并按原有保留期限妥善保存数据目录和加密密钥的恢复副本，再替换二进制。沿用原 `.env`、`DATA_DIR` 和 `DATA_ENCRYPTION_KEY`；不要重新生成加密密钥。启动时自动新增求职准备和账务表（schema version 3），已有诊断、会话及恢复码继续可用。
...
订单、账号和次数账本独立于报告保留期保存，已有付费数据上线后回滚程序需要先关闭购买并核对队列；旧程序不支持新的计次规则。详细配置与退款操作见 `docs/billing.md`，工程验收见 `docs/verification.md`。
