...
先等待生成队列空闲，再停止服务，并按原有保留期限妥善保存数据目录和加密密钥的恢复副本，然后替换二进制。沿用原 `.env`、`DATA_DIR` 和 `DATA_ENCRYPTION_KEY`；不要重新生成加密密钥。启动时自动升级到 schema version 5，新增求职准备、账务、新用户赠送及用户管理记录，已有诊断、会话、恢复码、订单和已购次数继续可用。
...
v5 增加 `billing_accounts.last_login_at`、`billing_account_controls` 和 `billing_account_audit`。管理入口 `/#admin/users` 复用原管理密钥，无需新增配置。旧账号默认正常使用，无管理备注和操作记录；最近登录时间从现有会话推算。迁移不会补发体验、限制账号或退出用户。核对旧账号、订单、余额及会话完整性后，用独立测试账号验证用户详情和管理操作；不要对真实账号执行验收变更。

订单、账号、次数账本和管理操作记录独立于报告保留期保存。v4 账本允许赠送记录不关联付款订单，v3 及更早程序不支持这一规则；v5 之前的程序也不会执行新增的账号生成限制。回滚必须同时考虑数据结构与权限语义，不能用升级前的副本覆盖上线后新增的账号、订单、消费或管理记录。应在恢复对外服务前核对迁移结果，成功后清理临时材料备份。详细配置与退款操作见 `docs/billing.md`，工程验收见 `docs/verification.md`。
